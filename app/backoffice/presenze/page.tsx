...
function getRigaCsv(valori: readonly string[]) {
  return valori.map(formattaCampoCsv).join(REPORT_PRESENZE_CSV.SEPARATORE);
}

function getValoriRigaCsv(riga: PresenzeReportRiga) {
  return [
    riga.data,
    riga.ora,
    riga.dipendente,
...
function creaCsv(righe: PresenzeReportRiga[]) {
  const contenuto = [
...
function scaricaCsv({
  ...
  const blob = new Blob([csv], { type: REPORT_PRESENZE_CSV.MIME_TYPE });
...
export default function BackofficePresenzePage() {
  ...
  const [filtri, setFiltri] = useState<PresenzeReportFiltri>({
    dipendenteId: null,
    cantiereId: null,
        ...
        actions={
          <>
            <Link href={APP_ROUTES.BACKOFFICE}>
              <Button variant="secondary" size="sm">{REPORT_PRESENZE_TESTI.BACKOFFICE}</Button>
            </Link>
            <Link href={APP_ROUTES.HOME}>
              <Button variant="secondary" size="sm">{REPORT_PRESENZE_TESTI.TIMBRATURE}</Button>
            </Link>
          </>
        }
      />
      ...
      <main className="mx-auto max-w-[1000px] px-6 py-6 print:max-w-none print:px-0 print:py-0">
        ...
        <div className="print:hidden">
          <nav aria-label="breadcrumb" className="mb-5 flex items-center gap-1.5 text-sm text-text-muted">
            ...
            <span>/</span>
            <span className="font-medium text-text-primary">{REPORT_PRESENZE_TESTI.TITOLO}</span>
          </nav>
          <h1 className="font-heading text-2xl font-medium text-text-primary">{REPORT_PRESENZE_TESTI.TITOLO}</h1>
          <p className="mt-1 text-sm text-text-muted">{REPORT_PRESENZE_TESTI.SOTTOTITOLO}</p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-text-muted">
                {REPORT_PRESENZE_TESTI.LIMITE_EXPORT_PREFIX}{" "}
                {REPORT_PRESENZE_LIMITI.MAX_GIORNI}{" "}
                {REPORT_PRESENZE_TESTI.LIMITE_EXPORT_GIORNI_E}{" "}
                {REPORT_PRESENZE_LIMITI.MAX_RIGHE}{" "}
                {REPORT_PRESENZE_TESTI.LIMITE_EXPORT_RIGHE_SUFFIX}
              </p>
        ...
        {report?.limiteRaggiunto && (
          <div className="mt-4 flex items-start gap-3 rounded-md border border-warning-500/30 bg-warning-50 px-4 py-3 text-sm text-warning-500 print:hidden">
            ...
            <p>{REPORT_PRESENZE_TESTI.LIMITE_RAGGIUNTO} ({report.limiteRighe}).</p>
          </div>
        )}
        ...
        <Card className="mt-4 p-5 print:border-0 print:p-0 print:shadow-none">
            ...
            <div>
              ...
              {report && (
                <p className="mt-0.5 text-xs text-text-muted">
                  {report.righe.length} {REPORT_PRESENZE_TESTI.RIGHE}
                </p>
              )}
            </div>
          </div>
          ...
          {!report && !loadingReport && (
            <p className="text-sm text-text-muted">{REPORT_PRESENZE_TESTI.NESSUNA_RICERCA}</p>
          )}
          {loadingReport && (
            <p className="text-sm text-text-muted">{REPORT_PRESENZE_TESTI.CARICAMENTO}</p>
          )}
          {report && report.righe.length === 0 && !loadingReport && (
            <p className="text-sm text-text-muted">{REPORT_PRESENZE_TESTI.NESSUN_RISULTATO}</p>
          )}
              ...
              <table className="min-w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border">
                    {REPORT_PRESENZE_COLONNE.map((colonna) => (
                      <th
                        key={colonna}
                ...
                <tbody>
                  {report.righe.map((riga) => (
                    <tr key={riga.id} className="border-b border-border last:border-b-0 hover:bg-bg-base transition-colors duration-150">
                      ...
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-muted">{riga.ora}</td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-primary">{riga.dipendente}</td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-muted">{riga.email}</td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-muted">{riga.tipoLabel}</td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-muted">{riga.destinazione}</td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-muted">{riga.cantiere}</td>
                      <td className="whitespace-nowrap px-3 py-2.5 text-text-muted">{riga.attivita}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
