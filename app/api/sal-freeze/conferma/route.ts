...
import { confermaSalFreeze } from "@/services/salFreeze/confermaSalFreeze";
...
const ERRORI_API = {
  ...
  FREEZE_GIA_ANNULLATO: "SAL periodo gia annullato",
  FREEZE_GIA_DEFINITIVO: "SAL periodo gia confermato",
  ERRORE_GENERICO: "Conferma SAL periodo fallita",
} as const;
...
async function leggiFreezeId(request: Request): Promise<string | null> {
  let payload: unknown;
  ...
  if (!isRecord(payload) || typeof payload.freezeId !== "string") {
    return null;
  }
...
function getErroreHttp(error: unknown): {
  status: number;
  errore: string;
} {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const freezeConfermato = await confermaSalFreeze({
      freezeId,
    ...
    return jsonOk({ freeze: freezeConfermato });
  } catch (error: unknown) {
