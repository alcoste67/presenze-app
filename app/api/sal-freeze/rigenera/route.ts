...
import { rigeneraSalFreeze } from "@/services/salFreeze/rigeneraSalFreeze";
...
const ERRORI_API = {
  ...
  FREEZE_GIA_DEFINITIVO: "SAL periodo definitivo: non rigenerabile",
  ERRORE_GENERICO: "Rigenerazione SAL periodo fallita",
} as const;
...
async function leggiFreezeId(request: Request): Promise<string | null> {
  ...
  return payload.freezeId.trim() || null;
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const freeze = await rigeneraSalFreeze({
      freezeId,
